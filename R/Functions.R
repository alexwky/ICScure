...
#' ## Generate 100 clusters with maximum size of 40
#' @export
...
ICDASim<-function(seed=NA,n,beta00,beta10,beta20,rho,gamma,cs){
  ...
  for(i in c(1:n)){
    ...
    t1<-rep(NA,length(Sij))
    if(rho==0){
      for(k in 1:length(Sij)){
        if(Sij[k]<cureij[k]){
          t1[k]<-999
        }else{
          t1[k]<--log(1-(Sij[k]^(-1)-1)*(1-exp(-4))/etaij[k])
        }
      }
    }else if (rho>0){
      for(k in 1:length(Sij)){
        if(Sij[k]<cureij[k]){
          ...
          t1[k]<--log(1-((-log(Sij[k])*rho+1)^(1/rho)-1)*(1-exp(-4))/etaij[k])
        }
      }
    }
    #if(rho==0){
...
#' @author Chun Yin (James) Lee <james-chun-yin.lee@polyu.edu.hk>
