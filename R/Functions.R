...
#'
#' @details The vector \code{psi} contains the coefficients of the Bernstein basis polynomial. The polynomial is supported on ( \code{mint} , \code{maxt} ) and is equal to zero at \code{mint}. The intercept should not be included in \code{psi}. The function returns the values of the polynomial at the time points in \code{t}.
#' @examples
#' t <- seq(1,10,1)
#' psi <- seq(0.25,1,0.25)
...
#' @param gamma Parameter \eqn{\gamma} in in the Gumbel Copula
...
#'
#' @param data A \code{n x (p+3)} matrix, where \code{n} is the sample size and \code{p} is the number of covariates. The first column consists of cluster indices, the second column consist of the observation time, the third column consist of the event indicator, and the forth to the last columns consist of the covariates (not including the intercept). The set of covariates can be empty.
#' @param rho Transformation parameter \eqn{\rho} in the Box-Cox transformation
...
#' @param t.min Left end point of the support of the distribution function \emph{\bold{F}}
#' @param t.max Cure threshold. If \code{NA}, then it is set to be the largest observation with \eqn{\Delta = 1}
#' @param reltol Relative tolerance for the function \code{optim} of the \code{stat} package
#' @param maxit Maximum number of iterations for the function \code{optim} of the \code{stat} package
...
#'   \item \strong{degree} : Degree of the Bernstein polynomial
#'   \item \strong{psi} : Vector of the coefficients of the Bernstein basis polynomial in the estimated distribution function \emph{\bold{F}}
#'   \item \strong{beta} : Vector of the regression parameters \eqn{\beta}
#'   \item \strong{betaSE} : Estimated standard error of the estimator for \eqn{\beta}
#'   \item \strong{iteration} : Numbers of iteration used the function \code{optim}
#'   \item \strong{covergence} : Logical; If TRUE, then the optimization algorithm is converged
#'   \item \strong{AIC} : Akaike information criterion (AIC) of the fitted model
#' }
#'
#' @author Wong Kin Yau, Alex <kin-yau.wong@polyu.edu.hk>
...
Est.ICScure <- function(data, rho, degree, weighting=TRUE, t.min=0, t.max=NA, reltol=10^-6, maxit=1000, Hessian=TRUE){
  ...
  if((sum(is.na(Hess))>=1)){
    ...
    results<-list(degree,psi,
                  ...
                  as.numeric(Maximization$counts[1]),as.character(convergence),AIC)
  }

  names(results)<-c("degree","psi","beta","betaSE","iteration","covergence","AIC")
