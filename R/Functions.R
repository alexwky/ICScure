...
#'
#' @details The degree of Bernstein polynomial is m. \eqn{\Psi}  is an (m+1)-dimentsional vector of coefficients of the basis polynomials. If \eqn{\Psi} does not be input, the values of psi will be calculated based on the parameter \eqn{\lambda}.
#'
#' @param t Values at which the Bernstein polynomial are evaluated
#' @param psi Vector of coefficients for the Bernstein basis polynomials. The dimension of the vector is equal to the degree plus 1
...
#'
#' @return Values of the Bernstein polynomial at t.
#' @export

BasisPolynomial.raw<-function(t,psi,mint,maxt){
    degree <- length(psi)
...
LOG.Lik.csw<-function(par,rho,degree,data,x0num,t.min,t.max){
  ...
  SiC<-ifelse(Ci<t.max,sapply(1:nn, function(o){Gpx(BasisPolynomial.raw(t=Ci[o], psi = psi, mint=t.min, maxt=t.max)*mu1[o],rho)}),
...
LOG.Lik.IJ.csw<-function(par,rho,degree,datline,x0num,t.min,t.max){ #data is in matrix format
  ...
  SiC<-ifelse(Ci<t.max,Gpx(BasisPolynomial.raw(t=Ci, psi = psi, mint=t.min, maxt=t.max)*mu1,rho),
...
#' Then, the event time for the ith cluster can be generated based on the Gumbel copula model with the join survival function.
#'
...
#'   \item \strong{family} : The nth cluster (subject)
#'   \item \strong{Ci} : Event time of interest
...
#'
#' @param data A matrix with n rows, where n is the number of observations. The first column is the nth cluster, the second column is event time of interest, The third column is event indicator \eqn{\delta}. The forth column to the end are the covariates (\eqn{x_1,x_2,...,x_n})
#' @param rho Transformation parameter \eqn{\rho}
...
#' @param t.max Cure threshold. If NA, then it is set to be the largest inspection time with \eqn{\Delta} = 1.
#' @param reltol Relative tolerance to input to the optimization function of R from "stats" package
