...
#'
#' @param data An \code{n x (p+3)} matrix, where \code{n} is the sample size and \code{p} is the number of covariates. The first column of data consists of cluster indices, the second column consists of the observation time, the third column consists of the event indicator, and the forth to the last columns consist of the covariates (not including the intercept). The set of covariates can be empty.
#' @param rho Transformation parameter \eqn{\rho} in the Box-Cox transformation
