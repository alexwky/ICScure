...
#'
#' @details The degree of Bernstein polynomial is m. Psi is an (m+1)-dimentsional vector of coefficients of the Bernstein polynomials. Then, this function calculate the values of the Bernstein polynomial at t.
#' @examples
#' t <- seq(1,10,1)
#' psi <- seq(0.25,1,0.25)
...
#'
#' @return A vector of values of the Bernstein polynomial at t.
#' @export
...
#'
#' @param data A n x (p+3) matrix, where n is the sample size and p is the numbers of covariates. The first column is the cluster number, the second column is observation time, The third column is event indicator. The forth column to the end are the covariates but not include the intercept. Data with no covariates is also allowed.
#' @param rho Transformation parameter \eqn{\rho}
